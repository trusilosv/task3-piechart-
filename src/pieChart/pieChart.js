function PieChart({ sectors, colorLine }) {
   ...
   const ListSectors = listValues.map((value) => {
      return (
         <div className='df' key={Math.random()}>
   ...
   const pie = sectors.map((sector, index) => {
      const { x1, x2, y1, y2, flag, radius, bisector } = sector;
      return (
         < ><path className='pieChart_sector' key={Math.random()}
            ...
            <circle cx={bisector.x + 20} cy={bisector.y + 20} r="6" stroke="black" strokeWidth="3" fill="red" /></>
      )
   })
   return (
      <>
         <svg key={Math.random()} className='pieChart' height={2 * sectors[0].radius * 1.25} width={2 * sectors[0].radius * 1.25} >
            {pie}
         </svg>
